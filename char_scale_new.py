...
class CharSCNN(object):
    def __init__(
        ...
        datatrainword0, datatestword, datatrainchar0, datatestchar, targettrain0, targettest = train_test_split(xwrd, xchr,y,random_state=1234, test_size=0.2)

        datatrainword, datavalidword, datatrainchar, datavalidchar, targettrain, targetvalid = train_test_split( datatrainword0, datatrainchar0,targettrain0,random_state=1234, test_size=0.1)
        ...
        xvalidword = theano.shared(np.asarray(datavalidword, dtype='int16'), borrow=True)
        xvalidchar = theano.shared(np.asarray(datavalidchar, dtype='int16'), borrow=True)
        yvalid = theano.shared(np.asarray(targetvalid, dtype='int8'), borrow=True)
        ...
        self.nvalidbatches1 = xvalidword.get_value(borrow=True).shape[0] / batchsize
...
#         # params = layer6.params + layer5.params + layer3.params + layer1.params + layercharembed.params
        ...
        updates = RMSprop(learningrate=0.002, params=params).updates(loss)
        ...
        self.validationmodel = theano.function(
            ...
            outputs=[loss, accuracy,layer6.output],
            givens={    
                xwrd: xvalidword[index*batchsize: (index+1)*batchsize],
                xchr: xvalidchar[index*batchsize: (index+1)*batchsize],
    ...
    def trainandtest(self, nepoch=4):
        ...
        while epoch < nepoch:
            ...
            for batchindex1 in range(int(self.ntrainbatches)):
                ...
                sumloss += batchloss_t
                sumaccuracy += batchaccuracy_t
            loss = sumloss / self.ntrainbatches
            accuracy = sumaccuracy / self.ntrainbatches
            print("train")
            print(loss)
            print(accuracy)
            ...
            print('epoch: {}, train batch loss={},train batch accuracy={}'.format(epoch,loss, accuracy))
            print('')
            ...
            print('epoch: {}, test mean loss={}, test mean accuracy={}'.format(epoch, loss, accuracy))
            print('')
            ...
            for batchindex3 in range(int(self.nvalidbatches1)):
                # print('test batch = {}'.format(batchindex2))
                batchloss, batchaccuracy,c = self.validationmodel(batchindex3)
                sumloss += batchloss
                sumaccuracy += batchaccuracy
            loss = sumloss / self.nvalidbatches1
            accuracy = sumaccuracy / self.nvalidbatches1
            valid_accuracies.append(accuracy)
            valid_losses.append(loss)
            print('epoch: {}, valid mean loss={}, valid mean accuracy={}'.format(epoch, loss, accuracy))
            print('')
        ...
        return test_accuracies , test_losses ,train_accuracies , train_losses , y_axis,self.ntrainbatches , valid_accuracies, valid_losses
if __name__ == '__main__':
    random_state = 1234
    rng = np.random.RandomState(random_state)
    charscnn = CharSCNN(rng, batchsize=100,activation=relu)
    test_accuracies,test_losses,train_accuracies , train_losses,y_axis,ntrainbatches,valid_accuracies, valid_losses = charscnn.trainandtest(nepoch=10)
    ...
    plt.plot(y_axis,valid_accuracies,label='valid accuracy')
    ...
    plt3.plot(y_axis,test_losses,label='test loss')
    plt3.plot(y_axis,train_losses,label='train loss')
    plt3.plot(y_axis,valid_losses,label='valid loss')
    # plt3.xlim([0,int(len(y_axis)/ntrainbatches)*len(y_axis)])
    # plt2.xlim([0,1000])
    plt3.ylim([0,1])
